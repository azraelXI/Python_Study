...
MessageList = ["Thank you!!!!! Thank you!!!!!", "Thanks for sharing!!", "Thanks for sharing!! Thanks!", "Thank you very much! Fierce!"]
...
#     task_resp.close()
#     time.sleep(1)
# 固定回复 签到帖:151106  指定签到任务：156809 晒妹帖：230589  &pid=39684869&page=6604&extra=
# replyurl = {"155106", "230589", "156809"}
# for reply in replyurl:
#     posturl = f"http://www.9dmdamaomod.net/forum.php?mod=viewthread&tid={reply}&pid=39684869&page=6604&extra="
...
# 回复帖子40次
obj = re.compile(r"thread-(?P<tid>.*?)-1", re.S)
...
#回复剩下的帖子
# for i in range(2, 4):
#     url = f"http://www.9dmdamaomod.net/forum.php?mod=forumdisplay&fid=365&filter=&orderby=dateline&page={i}&t=4643079"
#     resp = requests.get(url=url, headers=headers)
#     main_page = BeautifulSoup(resp.text, "html.parser")
#     divlist = main_page.find_all("div", class_="c cl")
#     for div in divlist:
...
#         child_resp = requests.post(posturl, headers=headers, data=data)
#         # print(child_resp.text)
#         if(str(child_resp.text).startswith('<root><![CDATA[抱歉，')):
#             child_resp.close()
#             time.sleep(3600)
#         else:
#             child_resp.close()
